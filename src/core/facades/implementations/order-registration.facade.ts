  ...
  implements OrderRegistrationFacadeInterface
{
  ...
  async createOrder(
    ...
    if (orders.STSOrder) {
      try {
        ...
        await this.inputStsUsageEnvironment(orders.STSOrder.orderNr);
        await this.inputStsModel(
    ...
    if (orders.INSOrder) {
      try {
        ...
        await this.inputInssUsageEnvironment(orders.INSOrder);

        await this.inputInssModel(orders.INSOrder);
  ...
  private async inputInssUsageEnvironment(order: INSSOrderModel) {
  ...
  private async inputStsUsageEnvironment(orderNr: string) {
    const endUserIsLoaded = await this.orderPuppeteer.checkLocation(
      '#order_enduser',
      false,
      false,
  ...
  private async inputInssModel(order: INSSOrderModel) {
    const isModelLoaded = await this.orderPuppeteer.checkLocation(
      '#insoleForm',
      false,
      false,
  ...
  private async inputStsModel(model: string, size: string, width: string) {
    const isModelLoaded = await this.orderPuppeteer.checkLocation(
      '#page-content-wrapper > div > div > div > div.col-md-7 > div',
      false,
      false,
